...
function init() {
    ...
    */
    ...
    //Добавляем элемент для счетчика
    screenScore = document.createElement('p');
    screenScore.innerHTML = "Счёт: "+ score;
    wrap.insertBefore(screenScore, wrap.firstChild);
...
function startGame() {
    ...
    // Добавляем таймер на запуск создания препядствий
...
function move() {
    ...
    /*
    // Проверка new_unit это стена
    else if (isWallUnit(new_unit)) {
        finishTheGame();
    }
    */
    else {
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        score++;
        // Выводим значение счетчика при изменении score
        screenScore.innerHTML = "Счет: "+score;
...
function createWall() {
    ...
    while (!wallCreated) { //пока стену не создали
        ...
        // проверка на змейку, проверку на еду
    ...
    if (gameIsRunning) {
        setTimeout(deleteWall, 3000, wall_cell);
    }
    
    
}

// Убираем стену и создаем новую
function deleteWall(wall_cell) {
    // убираем из полученного объекта класс 'wall-unit' и запускаем создание новой стены
    console.log(wall_cell);
    var classes = wall_cell.getAttribute('class').split(' ');
    wall_cell.setAttribute('class', classes[0] + ' ' + classes[1]);
    console.log(wall_cell);
    if (gameIsRunning) {
        setTimeout(createWall, 1000, wall_cell);
    }
